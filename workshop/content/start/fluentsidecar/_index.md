...
spec:
      ...
      - name: redis-statistic
        ...
        ....
```
Once its launched, Fluentbit is configured to run a custom script (loaded into the Fluentbit container path /bin/redis) to capture metrics from redis. It then forwards the data to Fluentd using the forward protocol that was pre-setup. You can find these configurations are in ```fluent-bit.conf```.
