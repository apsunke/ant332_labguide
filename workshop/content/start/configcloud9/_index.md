...
***WARNING:*** Please ensure that you follow steps for disabling of ***AWS Managed Temporary Credentials*** mentioned above, as the lab will ***NOT*** work if you fail to do so.
...
Ignore any warnings generated by the installer, they are usually bugs with the installer scripts.

Run the following command to ensure that the `helm` binary is installed correctly:

```
which helm
```

If helm is installed correctly, the output of this command should say the following:

```
/usr/local/bin/helm
```

If the output says `which: no helm in (/sbin:/bin:/usr/sbin:/usr/bin)` run the following commands from within the same directory to reinstall `helm`:
...
This should ensure correct installation of helm.
